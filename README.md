...
라이브클래스(온라인 강의 플랫폼)의 웹 서비스 이벤트 로그 파이프라인을 PostgreSQL + Docker Compose로 구현한 과제 프로젝트. **이벤트 생성 → 저장 → 사전 집계 → 분석**까지 단일 명령어로 자동화한다.
...
6. app 종료 (db는 계속 실행 → 분석 쿼리 수신 대기)
...
```
        ...
        │  │ users      │  │
        │  │ events     │  │  ← 비정규화 raw
        │  │ agg_event… │  │  ← (시·유저·타입) 사전 집계 wide fact
        │  └────────────┘  │
        └──────────────────┘
                 ▲
                 │ SELECT (분석 쿼리)
...
2. **db**: 데이터 보존, 5432 포트 노출, 외부 SQL 클라이언트 접속 대기
...
├── docker-compose.yml         # db + app 서비스 정의
├── README.md                  # 본 파일
...
- **시각화 미구현**: queries.sql 결과를 텍스트 표로 보는 데까지가 자동화 범위. Streamlit / Metabase / Grafana 후보 미결정.
